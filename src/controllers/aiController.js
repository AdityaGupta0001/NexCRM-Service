...
Return ONLY the JSON object as your response, without any surrounding text or explanations.
    ...
    try {
      ...
      res
        .status(500)
        .json({
          error: "AI returned a non-JSON response or invalid JSON.",
          details: aiResponse,
        });
    }
  } catch (error) {
    ...
    res
      .status(500)
      .json({
        error: "Failed to parse segment from prompt.",
...
const suggestMessages = async (req, res) => {
  try {
    const { objective, segmentDescription, tone } = req.body;
    if (!objective) {
...
Do not include any other text or explanations.
    ...
    try {
      ...
      if (
        !Array.isArray(messageSuggestions) ||
        !messageSuggestions.every((item) => typeof item === "string")
      ) {
      ...
      console.error(
        "Error parsing AI message suggestions as JSON array:",
        aiResponse
      );
      res
        .status(500)
        .json({
          error:
            "AI returned non-JSON or invalid format for message suggestions.",
          details: aiResponse,
        });
    }
  } catch (error) {
    ...
    res
      .status(500)
      .json({ error: "Failed to suggest messages.", details: error.message });
  }
};
